...
// Find ticket owned by user
async function getUserTicket(guildId, userId) {
  const result = await global.dbQuery(
    `
    SELECT *
    FROM tickets
...
async function sendTicketPanel(channel) {
  const embed = new EmbedBuilder()
    ...
    .setDescription(
      ...
      "Please explain your issue clearly and wait for our support team."
    )
    .setFooter({
      text: "Grand Mafia Support System"
    })
    .setTimestamp();
...
async function createTicket(interaction) {
  ...
  const existing = await getUserTicket(
    guild.id,
    user.id
  );
  ...
  const staffRole = config?.ticket_staff_role_id
    ? guild.roles.cache.get(config.ticket_staff_role_id)
    : null;

  const category = config?.ticket_category_id
  ...
  await global.dbQuery(
    `
    INSERT INTO tickets
    (guild_id, channel_id, user_id, status)
    VALUES ($1, $2, $3, 'open')
    `,
    [
      guild.id,
  ...
  await ticketChannel.send({
    content: staffRole ? `${staffRole}` : null,
    embeds: [embed],
    components: [row]
  });
...
async function closeTicket(interaction) {
  const ticket = await getTicketByChannel(
...
async function unclaimTicket(interaction) {
  ...
  if (ticket.claimed_by !== interaction.user.id) {
    return interaction.reply({
      content:
        "❌ You have not claimed this ticket.",
      ephemeral: true
    });
  }
...
async function addTicketMember(interaction) {
  const ticket = await getTicketByChannel(
    interaction.channel.id
  );
...
async function removeTicketMember(interaction) {
  const ticket = await getTicketByChannel(
    interaction.channel.id
  );
...
client.on("interactionCreate", async interaction => {
  try {
    ...
    if (interaction.commandName === "add") {
      return await addTicketMember(interaction);
    }

    if (interaction.commandName === "remove") {
      return await removeTicketMember(interaction);
    }
    ...
    if (interaction.commandName === "ticketsetup") {
      ...
      return interaction.reply({
        content:
          "⚙️ Ticket setup is ready. Ticket category and staff role can be configured through the server configuration system.",
        ephemeral: true
      });
    }

  } catch (error) {
    console.error("❌ Ticket system error:");
    console.error(error);
    ...
    try {
      if (interaction.replied || interaction.deferred) {
        await interaction.followUp({
          content:
            "❌ An error occurred in the ticket system.",
          ephemeral: true
        });
      } else {
        await interaction.reply({
...
console.log(
  "✅ Part 16 loaded — ticket system active."
);

// =========================
// PART 17 — AUTOMOD SYSTEM
// =========================
  ...
  try {
    ...
    if (!result.rows.length) {
      await global.dbQuery(
        ...
        (guild_id, spam_enabled, link_enabled,
         mention_enabled, filter_enabled, autotimeout_enabled)
        VALUES ($1, $2, $3, $4, $5, $6)
        ON CONFLICT (guild_id) DO NOTHING
        `,
        [
          guildId,
          DEFAULT_AUTOMOD.spam,
          DEFAULT_AUTOMOD.links,
          DEFAULT_AUTOMOD.mentions,
          DEFAULT_AUTOMOD.badWords,
          DEFAULT_AUTOMOD.autotimeout
        ]
      );

      return {
        spam_enabled: DEFAULT_AUTOMOD.spam,
        link_enabled: DEFAULT_AUTOMOD.links,
        mention_enabled: DEFAULT_AUTOMOD.mentions,
        filter_enabled: DEFAULT_AUTOMOD.badWords,
        autotimeout_enabled: DEFAULT_AUTOMOD.autotimeout
      };
    }
...
async function automodAction(message, type, reason) {
  ...
  try {
    ...
    if (type === "SPAM") timeoutMinutes = 5;
    if (type === "LINK") timeoutMinutes = 10;
    if (type === "MENTION") timeoutMinutes = 10;
    if (type === "BADWORD") timeoutMinutes = 5;
    ...
    if (typeof global.createModerationCase === "function") {
      await global.createModerationCase(
        message.guild.id,
        message.author.id,
...
client.on("messageCreate", async message => {
  try {
    ...
    if (
      ...
      containsBadWord(message.content)
    ) {
      return automodAction(
        message,
        "BADWORD",
...
client.on("interactionCreate", async interaction => {
  try {
    ...
    if (command === "automod") {
      ...
      return interaction.reply({
        content:
          `🛡️ **AutoMod:** ${enabled ? "Enabled" : "Disabled"}`
      });
    }
    ...
    if (command === "antimention") {
      ...
      return interaction.reply({
        content:
          `📢 **Anti-Mass-Mention:** ${enabled ? "Enabled" : "Disabled"}`
      });
    }
    ...
    if (command === "autotimeout") {
      ...
      return interaction.reply({
        content:
          `⏱️ **Auto-Timeout:** ${enabled ? "Enabled" : "Disabled"}`
      });
    }
